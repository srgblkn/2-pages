...
with st.sidebar:
    ...
    st.page_link("pages/1_blood.py", label="Анализ крови (по изображению)", icon="🩸")
    st.page_link("pages/2_sport.py", label="Определение вида спорта (по фото)", icon="🏅")
...
st.markdown(
    ...
    <div class="hero">
      ...
      <div class="h-sub">
        Два независимых модуля: распознавание типов клеток крови и определение вида спорта по фотографии.
        Интерфейс ориентирован на демонстрацию возможностей модели и понятную бизнес-аудитории визуализацию результатов.
      </div>
    </div>
    """,
    unsafe_allow_html=True,
...
with c2:
    with st.container(border=True):
        ...
        st.write(
            "Загрузка фотографии → определение наиболее вероятного вида спорта → Top-K вероятностей."
        )
